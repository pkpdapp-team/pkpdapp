...
def monolix_import(app_project: Project, project_str: str, model_str: str, data_str: str, validate: bool = True):
  """
  ...
  model, (admin_id, dosed_compartment, tlag, direct) = model_parser.parse(model_str)
  ...
  # set the parameters of the myokit model to match the typical population parameters
  for parameter_name, parameter in project['<MODEL>']['[INDIVIDUAL]']['DEFINITION:'].items():
  ...
  # get the right protocol (TODO: we cheat and only get the one for the first subject)
  ...
  model_variable = project['<MODEL>']['[LONGITUDINAL]']['DEFINITION:'][fit_variable]['prediction']
