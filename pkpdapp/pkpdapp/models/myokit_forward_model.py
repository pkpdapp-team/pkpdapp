...
class MyokitForwardModel:
    ...
    def __init__(
        ...
        self._output_names = [var.qname() for var in model.variables(const=False)]
        ...
        if self._subjects is not None:
            self._n_subjects = max([np.max(s_array) for s_array in self._subjects]) + 1
            self._times_all = []
            ...
            for s in range(self._n_subjects):
                ...
                self._output_indices.append(
                    [np.searchsorted(self._times_all[-1], t) for t in times_by_subject]
                )
        ...
        # times
            ...
            self._output_indices = [
                np.searchsorted(self._times_all, t) for t in self._times
            ]

        self._state_names = [var.qname() for var in model.states()]

        outputs_not_in_model = [v not in self._output_names for v in outputs]
        if any(outputs_not_in_model):
            raise ValueError(
                "All outputs must be within model. Outputs are:",
                outputs,
                ". Outputs in the model are:",
                self._output_names,
            )
        ...
        self._const_names = [var.qname() for var in model.variables(const=True)]
        ...
        if fixed_parameter_dict is None:
            ...
            fparams_not_in_model_params = [
                p not in self._all_parameter_names for p in fixed_parameter_dict.keys()
            ]
            if any(fparams_not_in_model_params):
                raise ValueError(
                    "All fixed parameter keys must correspond " "with model keys."
                )
            ...
            self._fixed_parameter_list = [
                self._fixed_parameter_dict[name] for name in self._fixed_parameter_names
            ]
            self._variable_parameter_names = [
                x
                for x in self._all_parameter_names
                if x not in self._fixed_parameter_names
            ]
            self._n_parameters = len(self._variable_parameter_names)
            self._fixed_parameter_indices = [
                self._all_parameter_names.index(v) for v in self._fixed_parameter_names
            ]

        self._variable_parameter_indices = [
            self._all_parameter_names.index(v) for v in self._variable_parameter_names
        ]
    ...
    def simulate(self, parameters):
        ...
        if self._subjects is not None and parameters.ndim != 2:
            raise ValueError(
                "subjects provided, but parameters shape is {}".format(parameters.shape)
            )

        if parameters.shape[0] != self._n_parameters:
            raise ValueError(
                "Dim 0 of parameters supplied must equal "
                + "number of non-fixed model parameters."
            )

        if self._subjects is not None and parameters.shape[1] != self._n_subjects:
            raise ValueError(
                "Dim 1 of of parameters supplied must equal " + "number of subjects."
            )
        ...
        if self._subjects is None:
            ...
            output = self._sim.run(t_max, log=self._output_names, log_times=log_times)
            ...
            if self._fixed_parameter_dict is None:
                ...
                full_parameters = np.empty((self._n_all_parameters, self._n_subjects))
            ...
            # preallocate results
            result = [np.empty_like(t) for t in self._times]
            for s in range(self._n_subjects):
                # Reset simulation
                self._sim.reset()
                ...
                output = self._sim.run(
                    t_max, log=self._output_names, log_times=log_times
                )
                ...
                # output_indices
                for output_index, (name, indices, subjects) in enumerate(
                    zip(
                        self._output_names,
                    ...
                    result[output_index][s == subjects] = np.array(output[name])[
                        indices
                    ]
