...
class Compound(models.Model):
    ...
    name = models.CharField(max_length=100, help_text="name of the compound")
    description = models.TextField(
        help_text="short description of the compound", blank=True, default=""
    )
    ...
    blood_to_plasma_ratio = models.FloatField(
        default=1.0, blank=True, null=True, help_text="blood to plasma ratio (unitless)"
    )

    intrinsic_clearance = models.FloatField(
        blank=True, null=True, help_text="intrinsic clearance"
    )
    ...
    target_molecular_mass = models.FloatField(
        default=25000.0,
        help_text=("molecular mass for target for conversion from mol to grams"),
    )
    ...
    target2_molecular_mass = models.FloatField(
        default=25000.0,
        help_text=("molecular mass for target 2 for conversion from mol to grams"),
    )

    target2_molecular_mass_unit = models.ForeignKey(
        Unit,
        on_delete=models.PROTECT,
        default=get_mol_mass_unit,
    ...
    target_concentration = models.FloatField(
        default=1.0, blank=True, null=True, help_text="target concentration"
    )
    ...
    dissociation_constant = models.FloatField(
        blank=True, null=True, help_text="dissociation constant"
    )
    ...
    is_soluble = models.BooleanField(
        default=True, help_text="is the compound target soluble"
    )
    ...
    def copy(self):
        kwargs = {
            ...
            "target2_molecular_mass": self.target2_molecular_mass,
            "target2_molecular_mass_unit": self.target2_molecular_mass_unit,
        }
