...
class TestInferenceWizardView(APITestCase):
    ...
    def test_population_and_covariates_inference(self):
        (
            ...
            pd_model,
            pd_dataset,
        ) = create_pd_inference(sampling=False)
        ...
        pd_parameter_names = [v.qname for v in pd_model.variables.filter(constant=True)]
        data = {
            ...
            "parameters": [
                {
                    ...
                    "parameters": [
                        (
                            r'0.1 * biomarker("{}") + '
                            r'parameter("population_parameter")'.format(
                                pd_biomarker_names[1]
                            )
                        ),
                        0.1,
                    ],
                },
                {"name": "population_parameter", "form": "N", "parameters": [1, 0.1]},
            ],
            # output
            "observations": [
                {
        ...
        for ll in response_data["log_likelihoods"]:
            if ll["name"] == "mean for " + pd_parameter_names[0]:
                ...
                self.assertEqual(len(ll["parameters"]), 2)
                self.assertTrue(
                    ll["description"] == "0.1 * arg0 + arg1"
                    or ll["description"] == "0.1 * arg1 + arg0"
                )
        self.assertTrue(found_it)
        ...
        log_posterior(log_posterior.to_search([0.5] * log_posterior.n_parameters()))
    ...
    def test_pd_inference_runs(self):
        ...
        for ll in response_data["log_likelihoods"]:
            if ll["name"] == pd_output_name:
                found_it = True
                self.assertEqual(len(ll["parameters"]), 2)
                self.assertEqual(ll["parameters"][0]["name"], pd_output_name)
        ...
        log_posterior(log_posterior.to_search([0.5, 0.12, 0.1]))
    ...
    def test_errors(self):
        ...
        response = self.client.post("/api/inference/wizard", data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("Expected", response.data["parameters"][0]["parameters"][0])
        self.assertIn(
            "not in list of parameters", response.data["parameters"][1]["parameters"][0]
        )
        self.assertIn(
            "not in list of biomarkers", response.data["parameters"][2]["parameters"][0]
        )
