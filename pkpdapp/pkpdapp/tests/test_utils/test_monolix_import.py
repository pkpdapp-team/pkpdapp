...
from pkpdapp.utils import monolix_import
...
class TestMonolixImport(TestCase):
    ...
    def test_import_project(self):
        ...
        constant_variables = pd_model.variables.filter(constant=True).values_list(
            "qname", flat=True
        )
        ...
        state_variables = pd_model.variables.filter(state=True).values_list(
            "qname", flat=True
        )
        ...
        displayed_biomarkers = dataset.biomarker_types.filter(display=True).values_list(
            "name", flat=True
        )
        ...
        pd_states = pd_model.variables.filter(state=True).values_list(
            "qname", flat=True
        )
        pk_states = pk_model.variables.filter(state=True).values_list(
            "qname", flat=True
        )
