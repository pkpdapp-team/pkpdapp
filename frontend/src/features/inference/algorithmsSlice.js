...
export const fetchAlgorithms = createAsyncThunk('algorithms/fetchAlgorithms', async (project, { getState }) => {
...
export const updateAlgorithm = createAsyncThunk(
  ...
  async (algorithm) => {
    const response = await api.put(`/api/algorithm/${algorithm.id}/`, algorithm)
    return response
  }
)
