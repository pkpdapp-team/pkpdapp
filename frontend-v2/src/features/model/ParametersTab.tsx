...
import { CombinedModel, CombinedModelRead, CompoundRead, ProjectRead, UnitRead, VariableRead, useCombinedModelSetParamsToDefaultsUpdateMutation, useCompoundRetrieveQuery, usePharmacokineticRetrieveQuery, useUnitListQuery, useVariableUpdateMutation } from '../../app/backendApi';
...
import { getConstVariables, getNoReset, resetToSpeciesDefaults } from './resetToSpeciesDefaults';
...
const ParametersTab: React.FC<Props> = ({ model, project, control, variables, compound, units }) => {
    ...
    const aaIsNotDosed = variables.filter((variable) => variable.protocol && variable.name === 'Aa').length === 0;
    const willFilterFandKa = model.is_library_model && aaIsNotDosed;
    let constVariables = getConstVariables(variables, model);
    if (willFilterFandKa) {
      constVariables = constVariables.filter((variable) => !['F', 'ka'].includes(variable.name));
    }
    ...
    return (
      ...
      <Tooltip title='No default parameters as "Other" has been selected as species (in "Projects")'>
        <span><Button variant="contained" color="primary" onClick={myResetToSpeciesDefaults} disabled={noReset} sx={{width: 270}}>Reset to Species Defaults</Button></span>
      </Tooltip>
      ) : (
        <Button variant="contained" color="primary" onClick={myResetToSpeciesDefaults} disabled={noReset} sx={{width: 270}}>Reset to Species Defaults</Button>
      )}
      <TableContainer>
      <Table>
        <TableHead>
